...
export default function AIWidget({ context }: { context?: string }) {
  ...
  return (
    <>
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed bottom-6 right-6 w-80 sm:w-96 h-96 bg-white border border-slate-200 rounded-2xl shadow-2xl flex flex-col z-50 overflow-hidden transform transition-all">
          ...
          <form onSubmit={sendMessage} className="border-t border-slate-200 p-3 bg-white flex gap-2">
            ...
            />
            <button disabled={loading || !input.trim()} type="submit" className="flex items-center justify-center p-2 bg-indigo-600 text-white rounded-full hover:bg-indigo-700 disabled:opacity-50 transition">
